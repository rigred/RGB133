...
typedef struct _sVWWriteFlash {
   ...
   /* VHDL version with date packed into a 32-bit number */
   /* This structure had data field 'data' here before BASE_VHDL_PAD_VER  */
   unsigned int   version;

   /* The 'data' field must be 64-bit aligned,
    * which can affect flash writes for the VisionAV family
    * on 64-bit systems
    */
   unsigned char  data[];
} sVWWriteFlash;
